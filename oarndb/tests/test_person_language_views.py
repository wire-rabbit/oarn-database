...
class PersonLanguageViewsTestCase(APITestCase):
    ...
    def setUp(self):
        ...
        self.fbb_post_data = {
            "person": self.factory.new_person(self.factory.fbb, has_language=False).pk,
            "ref_language": self.factory.language_english.pk,
            "ref_language_use_type": self.factory.language_use_type_home.pk,
            "other_details": "Native english speaker"
        }

        self.mtn_post_data = {
            "person": self.factory.new_person(self.factory.mtn, has_language=False).pk,
            "ref_language": self.factory.language_english.pk,
            "ref_language_use_type": self.factory.language_use_type_home.pk,
            "other_details": "Native english speaker"
        }
        ...
        self.fbb_patch_data = {
            "other_details": "Non-native english speaker"
        }

    def test_globalreadonly_can_get_all_language_records(self):
        """
        ...
        self.assertTrue(PersonLanguage.objects.filter(
    ...
    def test_globaladmin_can_get_all_language_records(self):
        """
        The basic family created by the factory consists of one adult and one child.
        Do we get 8 records back?
    ...
    def test_fbb_readonly_can_get_fbb_language_records(self):
        """
        The basic family created by the factory consists of one adult and one child.
        ...
        self.assertFalse(PersonLanguage.objects.filter(
            person__adult__adultfamilyrelationship__family=self.mtn_family_a).filter(
            person_id__in=ids)
        )
    ...
    def test_fbb_readwrite_can_get_fbb_language_records(self):
        """
        The basic family created by the factory consists of one adult and one child.
        Do we get 4 records back?
    ...
    def test_fbb_admin_can_get_fbb_language_records(self):
        """
        The basic family created by the factory consists of one adult and one child.
        Do we get 4 records back?
    ...
    def test_global_readonly_cannot_create_an_fbb_language_record(self):
        create_url = reverse('language-list')

        response = self.factory.global_readonly_client.post(create_url, self.fbb_post_data, format='json')
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)

    def test_global_admin_can_create_an_fbb_language_record(self):
        create_url = reverse('language-list')

        response = self.factory.global_admin_client.post(create_url, self.fbb_post_data, format='json')
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)

    def test_fbb_readonly_cannot_create_an_fbb_language_record(self):
        create_url = reverse('language-list')

        response = self.factory.fbb_readonly_client.post(create_url, self.fbb_post_data, format='json')
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)

    def test_fbb_readonly_cannot_create_a_mtn_language_record(self):
        create_url = reverse('language-list')

        response = self.factory.fbb_readonly_client.post(create_url, self.mtn_post_data, format='json')
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)
        
    def test_fbb_readwrite_can_create_an_fbb_language_record(self):
        create_url = reverse('language-list')

        response = self.factory.fbb_readwrite_client.post(create_url, self.fbb_post_data, format='json')
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
        
    def test_fbb_readwrite_cannot_create_a_mtn_language_record(self):
        create_url = reverse('language-list')

        response = self.factory.fbb_readwrite_client.post(create_url, self.mtn_post_data, format='json')
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)

    def test_fbb_admin_can_create_an_fbb_language_record(self):
        create_url = reverse('language-list')

        response = self.factory.fbb_admin_client.post(create_url, self.fbb_post_data, format='json')
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)

    def test_fbb_admin_cannot_create_a_mtn_language_record(self):
        create_url = reverse('language-list')

        response = self.factory.fbb_admin_client.post(create_url, self.mtn_post_data, format='json')
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)

    def test_global_readonly_cannot_update_an_fbb_language_record(self):
        np = self.factory.new_person(self.factory.fbb, has_language=True)
        ld = np.personlanguage_set.all().first()
        detail_url = reverse('language-detail', args=[ld.pk])
    ...
    def test_global_admin_can_update_an_fbb_language_record(self):
        np = self.factory.new_person(self.factory.fbb, has_language=True)
        ld = np.personlanguage_set.all().first()
        detail_url = reverse('language-detail', args=[ld.pk])
    ...
    def test_fbb_readonly_cannot_update_an_fbb_language_record(self):
        np = self.factory.new_person(self.factory.fbb, has_language=True)
        ld = np.personlanguage_set.all().first()
        detail_url = reverse('language-detail', args=[ld.pk])
    ...
    def test_fbb_readwrite_can_update_an_fbb_language_record(self):
        np = self.factory.new_person(self.factory.fbb, has_language=True)
        ld = np.personlanguage_set.all().first()
        detail_url = reverse('language-detail', args=[ld.pk])
    ...
    def test_fbb_admin_can_update_an_fbb_language_record(self):
        np = self.factory.new_person(self.factory.fbb, has_language=True)
        ld = np.personlanguage_set.all().first()
        detail_url = reverse('language-detail', args=[ld.pk])
    ...
    def test_global_readonly_cannot_delete_an_fbb_language_record(self):
        np = self.factory.new_person(self.factory.fbb, has_language=True)
        ld = np.personlanguage_set.all().first()
        detail_url = reverse('language-detail', args=[ld.pk])
    ...
    def test_global_admin_can_delete_an_fbb_language_record(self):
        np = self.factory.new_person(self.factory.fbb, has_language=True)
        ld = np.personlanguage_set.all().first()
        detail_url = reverse('language-detail', args=[ld.pk])
    ...
    def test_fbb_readonly_cannot_delete_an_fbb_language_record(self):
        np = self.factory.new_person(self.factory.fbb, has_language=True)
        ld = np.personlanguage_set.all().first()
        detail_url = reverse('language-detail', args=[ld.pk])
    ...
    def test_fbb_readwrite_can_delete_an_fbb_language_record(self):
        np = self.factory.new_person(self.factory.fbb, has_language=True)
        ld = np.personlanguage_set.all().first()
        detail_url = reverse('language-detail', args=[ld.pk])
    ...
    def test_fbb_admin_can_delete_an_fbb_language_record(self):
        np = self.factory.new_person(self.factory.fbb, has_language=True)
        ld = np.personlanguage_set.all().first()
        detail_url = reverse('language-detail', args=[ld.pk])
